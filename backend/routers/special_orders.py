...
async def create_special_order(payload: SpecialOrderCreate, request: Request) -> Dict[str, Any]:
    ...
    if initial_status == "pending_approval":
        try:
            ...
            need_role = str(special_order.get("required_approval_role") or "")
            await create_addressed(
                permission=("order", "approve"),
                roles=((need_role,) if need_role else ()),
                notif_type="special_order_approval",
                ref=f"so_custom_appr:{order_id}",
                title=f"PO custom menunggu persetujuan: {order_number}",
                body=(f"{special_order.get('customer_name', '')} · "
                      f"{rupiah(float(total_amount or 0))}. Diajukan oleh "
                      f"{user.get('name', '')}. Perlu persetujuan "
